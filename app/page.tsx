...
export default function Page() {
        ...
        <div className="mx-auto flex min-h-[80vh] max-w-4xl flex-col items-center justify-center text-center">
          ...
          <div className="mt-6 flex items-center justify-center gap-4">
            <label className="flex items-center gap-3 rounded-2xl border border-border bg-card p-3">
              <span className="text-sm text-muted-foreground">Tiempo por respuesta</span>
              <select className="rounded-lg bg-background px-3 py-2 text-sm" value={timePerQuestion} onChange={(e) => setTimePerQuestion(Number(e.target.value))}>
  ...
  return <GameRoom mode="screen" room={initialCode} timePerQuestion={timePerQuestion} />
}
